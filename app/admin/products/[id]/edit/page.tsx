...
export default function EditProductPage() {
  ...
  const fetchProduct = async () => {
    try {
      ...
      // Build features object with custom labels/descriptions
      const featuresMap: ProductForm["features"] = {
        premium_cotton: {
          enabled: false,
          custom_label: "",
          custom_description: "",
      ...
      product.features?.forEach((feature: any) => {
        if (featuresMap[feature.feature_type as keyof typeof featuresMap]) {
          featuresMap[feature.feature_type as keyof typeof featuresMap] = {
            enabled: true,
      ...
      setFormData({
        ...
        images: [],
        size_card_template_id: product.size_card_template_id?.toString() || "",
        features: featuresMap,
      });

      // Load existing images
      if (product.images && product.images.length > 0) {
        const imagePreviews = product.images
          .sort((a: any, b: any) => a.order - b.order)
          .map((img: any) => img.image_url);
        setImagePreviews(imagePreviews);
      } else if (product.image_url) {
        // Fallback for old products with single image_url
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Include existing previews that are URLs (not new files)
        ...
        <div className="p-4 md:p-6 max-w-4xl">
          ...
          <div className="flex items-center gap-4 mb-8">
            ...
            <div>
              ...
              <p className="text-white/60 mt-1">Update product information</p>
            </div>
          </div>
              ...
              <div className="space-y-4">
                ...
                {imagePreviews.length > 0 ? (
                  <div>
                      ...
                      {imagePreviews.map((preview, idx) => (
                        ...
                        >
                          ...
                          <button
                            ...
                            className="absolute top-1 right-1 px-2 py-1 bg-red-500/20 hover:bg-red-500/30 border border-red-400/30 text-red-300 rounded text-xs transition-all"
                          >
                            Remove
                          </button>
                        </div>
                    ...
                    <label className="flex items-center justify-center w-full h-32 rounded-xl border-2 border-dashed border-white/20 hover:border-white/40 cursor-pointer bg-white/5 hover:bg-white/10 transition-all">
                    ...
                    >
                      ...
                      {sizeCardTemplates.map((template) => (
                        <option key={template.id} value={template.id}>
                          {template.name} ({template.rows?.length || 0} sizes)
                        </option>
                      ))}
                    </select>
                  )}
